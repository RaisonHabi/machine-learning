...
概率累积会出现数值非常小的情况，比如1e-30，由于计算机的精度是有限的，
无法识别这一类数据，取对数之后，更易于计算机的识别(1e-30以10为底取对数后便得到-30)。
```
[人人都懂EM算法](https://zhuanlan.zhihu.com/p/36331115)

&nbsp;
### lr、xgb优缺点
lr：模型简单，可解释性强，易并行和工程化，ftrl能实时处理高纬稀疏问题。缺点是手工特征工程    
xgb：特征工程相对较少，模型效果好。缺点，可解释性略差，模型相对复杂，部署相对复杂，
